...
const handleRetryProcessing = async () => {
  if (!selectedProject) return;
  try {
    const res = await retryRfpProcessing(selectedProject.id, token);
    alert(res.message);

    setProcessingStatus("Retry started...");

    // Update project status to queued
    setSelectedProject(prev => prev ? { ...prev, status: "queued" } : null);
    ...
    setProcessingStatus("Retry failed");
  }
};
 ...
 const handleStartProcessing = async () => {
  ...
  <FaDownload /> Download Document
                  ...
                  {activeTab === "proposal" && (
                    <pre>{selectedProject.generated_proposal || "Proposal not generated yet."}</pre>
                  )}
                </div>
              </div>
            </div>
