...
const TemplatesPage: React.FC = () => {
  ...
  const [queueOverview, setQueueOverview] = useState<any>({ active_tasks: 0, pending_tasks: 0, failed_tasks: 0 });
  ...
  // Fetch processing queue overview on mount
  useEffect(() => {
    const fetchQueueOverview = async () => {
      try {
        const overview = await getRfpQueueOverview(token);
        setQueueOverview(overview || { active_tasks: 0, pending_tasks: 0, failed_tasks: 0 });
      } catch {
        setQueueOverview({ active_tasks: 0, pending_tasks: 0, failed_tasks: 0 });
      }
    };
    fetchQueueOverview();
  }, [token]);
  ...
  return (
    <div className="templates-page">
      ...
      <div className="templates-content">
        ...
        {/* Queue overview */}
        <div className="queue-overview-summary">
          <span>Active: {queueOverview.active_tasks ?? 0}</span>
          <span>Pending: {queueOverview.pending_tasks ?? 0}</span>
          <span>Failed: {queueOverview.failed_tasks ?? 0}</span>
        </div>
              ...
              {filteredProjects.map((p) => (
                <div key={p.id} className="project-list-item">
                  ...
                  <div className="list-item-right">
                    <button onClick={() => handleViewDetails(p.id)} className="view-btn"><FaEye /> View</button>
                    <button onClick={() => handleDelete(p.id)} className="delete-btn"><FaTrash /> Delete</button>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div className="modal-content">
                  {activeTab === "info" && (
                    <>
                      ...
                      {processingStatus && (
                        <p className="processing-response"><strong>Processing:</strong> {processingStatus}</p>
                      )}

                      {/* Export button */}
                      <div className="export-dropdown-wrapper">
                        <button className="primary-btn" onClick={() => setShowExportOptions(!showExportOptions)}>Export</button>
                        {showExportOptions && (
                          <div className="export-dropdown">
                            <button onClick={() => handleProposalDownload(selectedProject.id, "pdf")}>PDF</button>
                            <button onClick={() => handleProposalDownload(selectedProject.id, "docx")}>DOCX</button>
                          </div>
                        )}
                      </div>
                    </>
                  ...
                  {activeTab === "document" && (
                    <div className="document-section">
                      ...
                      {selectedProject.file_download_url ? (
                        <button className="download-btn" onClick={handleDownload}><FaDownload /> Download Document</button>
                        ...
                        <>
                          <div className="proposal-header">
                            ...
                            <div className="proposal-download-buttons">
                              <button className="download-btn" onClick={() => handleProposalDownload(selectedProject.id, "pdf")}><FaDownload /> PDF</button>
                              <button className="download-btn" onClick={() => handleProposalDownload(selectedProject.id, "docx")}><FaDownload /> DOCX</button>
                            </div>
                          </div>
                          <div className="proposal-text" dangerouslySetInnerHTML={{ __html: selectedProject.generated_proposal
...
//           dangerouslySetInnerHTML={{
//             __html: selectedProject.generated_proposal
